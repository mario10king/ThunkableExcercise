    ...
    return(
      <div className={style["outer-modal"]}>
        <div className={style["inner-modal"]}>
          ...
          <button className={style["no-button"]} onClick={(event)=>props.handleNo(event)}>No</button>
          <button className={style["yes-button"]} onClick={(event)=>props.handleYes(event, props.id)}>Yes</button>
        </div>
      </div>
    )
}
